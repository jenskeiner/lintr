...
class Rule(ABC):
    ...
    def can_fix(self) -> bool:
        ...
        Returns:
            True if the rule supports auto-fixing, False otherwise.
        """
        pass

    @abstractmethod
    def fix(self, repo: Repository) -> bool:
        """Apply the fix for this rule to the repository.
        
        This method should only be called if can_fix() returns True.
        
        Args:
            repo: GitHub repository to fix.
...
class RuleSet:
    ...
    def add_rule(self, rule: Rule) -> None:
        ...
        if rule.rule_id in self._rules:
            raise ValueError(f"Rule with ID {rule.rule_id} already exists in this rule set")
    ...
    def add_rule_set(self, rule_set: 'RuleSet') -> None:
        ...
        if rule_set.rule_set_id in self._rule_sets:
            raise ValueError(f"Rule set with ID {rule_set.rule_set_id} already exists in this rule set")
