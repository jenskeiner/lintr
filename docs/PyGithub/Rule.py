...
class Rule(NonCompletableGithubObject):
    ...
    def _useAttributes(self, attributes: dict[str, Any]) -> None:
        if "ruleset_source_type" in attributes:  # pragma no branch
            self._ruleset_source_type = self._makeStringAttribute(
                attributes["ruleset_source_type"]
            )
        if "ruleset_source" in attributes:  # pragma no branch
            self._ruleset_source = self._makeStringAttribute(
                attributes["ruleset_source"]
            )
