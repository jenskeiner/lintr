...
class Ruleset(CompletableGithubObject):
    ...
    def update(
        ...
        headers, data = self._requester.requestJsonAndCheck(
            "PUT", self.url, input=put_parameters
        )
    ...
    def _useAttributes(self, attributes: dict[str, Any]) -> None:
        ...
        if "bypass_actors" in attributes:  # pragma no branch
            self._bypass_actors = self._makeListOfDictsAttribute(
                attributes["bypass_actors"]
            )
        ...
        if "current_user_can_bypass" in attributes:  # pragma no branch
            self._current_user_can_bypass = self._makeStringAttribute(
                attributes["current_user_can_bypass"]
            )
        if (
            "_links" in attributes
            and "self" in attributes["_links"]
            and "href" in attributes["_links"]["self"]
        ):  # pragma no branch
